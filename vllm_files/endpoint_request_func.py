...
from .custom_scheduler import ModelScheduler 
import urllib
...
async def async_request_openai_completions(
    ...
    # Define the coro_factory that the scheduler will call when it selects the model.
    async def coro_factory() -> RequestFuncOutput:
    ...
    # HUZI: Use the custom scheduler
    if custom_scheduler:
        if scheduler is None:
            raise ValueError("custom_scheduler is True but no scheduler instance provided")
        # Figure out model name used to route
        target_model_name = request_func_input.model_name
        output: RequestFuncOutput = await scheduler.schedule(coro_factory, target_model_name)
    else:
        output: RequestFuncOutput = await coro_factory()
