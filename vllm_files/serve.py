...
from .lib.custom_scheduler import ModelScheduler 
...
# HUZI: New function to parse timestamps
def parse_timestamp(ts: Union[float, str]) -> float:
    """Parse a timestamp that can be either a float (epoch milliseconds) or an ISO string.
    Returns seconds since epoch as float (UTC).
    """
    if isinstance(ts, (int, float)):
        return float(ts / 1000)
    if isinstance(ts, str):
        # Accept ISO format; support trailing Z
        s = ts
        if s.endswith("Z"):
            s = s[:-1] + "+00:00"
        ...
        return dt.timestamp()
    raise ValueError("Unsupported timestamp format; expected float or ISO string")

# HUZI: New funcion to replace port
def replace_port(url, new_port):
    ...
    if p.username:
        if p.password:
            ...
            userinfo = f"{p.username}@"

    new_netloc = f"{userinfo}{p.hostname}:{new_port}"

    return urlunparse(p._replace(netloc=new_netloc))

# HUZI: New funcion to read model to port map
def read_model_to_port_map(path):
    model_to_port_map = {}
    try: 
        ...
        raise json.JSONDecodeError(f"The file '{path}' is not valid JSON!")
    
    return model_to_port_map
...
def calculate_metrics(
    ...
    Args:
        ...
        outputs: The outputs of the requests.
        dur_s: The duration of the benchmark.
        tokenizers: The tokenizers to use.
        selected_percentiles: The percentiles to select.
        goodput_config_dict: The goodput configuration.
        custom_scheduler: Whether to use custom scheduler or not
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            if custom_scheduler:
                # HUZI: Get correct tokenizer
                model_name = input_requests[i].model_name
...
async def benchmark(
    ...
    if custom_scheduler:
        ...
        scheduler = ModelScheduler(
            base_url=api_url,
            model_to_port_map=model_to_port_map,
            session=session,
    ...
    async def limited_request_func(
        ...
        if custom_scheduler:     
            ...
            # Get port
            if not raw_request.model_name in model_to_port_map:
                raise KeyError(f"File '{model_to_port_map_file}' does not have Key {raw_request.model_name}")
            new_port = model_to_port_map[raw_request.model_name]
            # Change api_url port
    ...
    # HUZI: If timestamps are provided and requested, schedule by timestamps
    if use_timestamp:
        # ensure all requests have a timestamp
        for req in input_requests:
            if req.timestamp is None:
                raise ValueError("use_timestamp=True but at least one request is missing 'timestamp'.")
        # parse timestamps and sort requests by timestamp
        parsed = [(req, parse_timestamp(req.timestamp)) for req in input_requests]
        parsed.sort(key=lambda x: x[1])
        first_ts = parsed[0][1]
        start_ts = time.time()

        for req, ts in parsed:
            delay_from_first = ts - first_ts
            if delay_from_first < 0:
                # allow negative (in the past) by treating as immediate
                delay_from_first = 0
            send_at = start_ts + delay_from_first
            sleep_interval = send_at - time.time()
            if sleep_interval > 0:
    ...
    if task_type == TaskType.GENERATION:
        metrics, actual_output_lens = calculate_metrics(
            ...
            tokenizers=tokenizers, # HUZI
            selected_percentiles=selected_percentiles,
            goodput_config_dict=goodput_config_dict,
            custom_scheduler=custom_scheduler, # HUZI
        )
    else:
...
def add_cli_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--model",
        type=str,
        # required=True, HUZI: if --custome-scheduler is enabled, this is not required
    ...
    # HUZI: New argument to use timestamps from input requests
    parser.add_argument(
        "--use-timestamp",
        action="store_true",
        help="If specified, the benchmark will schedule requests "
        "based on the 'timestamp' field in each input request."
    )
    
    # HUZI: New arguments for custom scheduler
    parser.add_argument(
        "--custom-scheduler",
        action="store_true",
        help="If specified, use HUZI custom scheduler."
    )
    parser.add_argument(
        "--model-to-port-map-file",
        type=str,
        default=None,
        help="Specify path to JSON file where models are "
        "mapped to a certain port.",
    )
    parser.add_argument(
        "--sleep-level",
        type=int,
        default=2,
        choices=[1, 2],
        help="Specify sleep/wakeup level (default: 2)",
    )
    parser.add_argument(
        "--check-every",
        type=float,
        default=0.1,
        help="Specifies the time interval (s) for the scheduler "
        "to poll the status of the server (default: 0.1 second)."
    )
    parser.add_argument(
        "--wakeup-timeout",
        type=float,
...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.custom_scheduler:
        ...
        for model in model_to_port_map.keys():
            tokenizers[model] = None
    else:
        tokenizer_id = args.tokenizer if args.tokenizer is not None else args.model
        tokenizers[tokenizer_id] = None
    ...
    for tok_id in tokenizers.keys():          
        tokenizer = get_tokenizer(
            tok_id,
            tokenizer_mode=args.tokenizer_mode,
            trust_remote_code=args.trust_remote_code
        )
        tokenizers[tok_id] = tokenizer
    ...
    # Load the dataset.
    input_requests = get_samples(args, tokenizers)
    ...
    result_json["tokenizer_ids"] = ",".join(tokenizers.keys())
