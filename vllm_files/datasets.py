...
class SampleRequest:
    ...
    # HUZI: New optional fields
    timestamp: Optional[Union[float, str]] = None  # unix epoch seconds (float) or ISO string
...
def get_samples(args, tokenizers) -> list[SampleRequest]:
    ...
    if args.dataset_name == "custom":
        ...
        input_requests = dataset.sample(
            ...
            tokenizer=tokenizers, # HUZI: Pass all tokenizers
        ...
        **kwargs,
        ...
        for i, item in enumerate(self.data):
            ...
            if custom_scheduler:
                ...
                if model_name not in tokenizer:
                    raise KeyError(
                        f"{model_name} not found in tokenizer. "
                        "Make sure it's included in the model_to_port map JSON file"
                    )
            ...
            sampled_requests.append(
                SampleRequest(
                    prompt=prompt,
                    prompt_len=prompt_len,
                    expected_output_len=item["output_length"] if item.get("output_length", None) else output_len, # HUZI
                    timestamp=item.get("timestamp", None), # HUZI
                    model_name=item.get("model_name", None), # HUZI
                    request_id=request_id_prefix + str(i),
                ))
